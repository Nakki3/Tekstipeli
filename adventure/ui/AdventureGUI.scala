...
object AdventureGUI extends SimpleSwingApplication {
  ...
  def top = new MainFrame {
    ...
    //editoi uusi playTurn tyyppinen ratkaisu niin että kyseessä on taistelu. pelaajalla on isInBattle ominaisuus

    def playTurn(command: String) = {
      
      val turnReport = if(!this.player.isInBattle) this.game.playTurn(command) else this.game.playBattleTurn(command)
